...
כתוב ניתוח רציף בעברית — פסקה אחת ארוכה ללא כותרות, ללא סוגריים מרובעים, ללא bullet points. 
הניתוח יכלול: מצב הנבחרות, כוח ההתקפה וההגנה, שחקנים מרכזיים שכדאי לעקוב אחריהם, עייפות/ימים מהמשחק הקודם, ומשמעות המשחק לטורניר.
לאחר הפסקה הוסף שורה אחת בפורמט: תוצאה צפויה: X-Y ל[שם הנבחרת]

חשוב: אל תכתוב [עדכונים], [תחזית], ##, או כל כותרת אחרת. רק פסקה + שורת תחזית.`;
    ...
    try {
        ...
        // Parse: פסקה ראשית + שורת תחזית
        const lines = text.split('\n').map(l => l.trim()).filter(Boolean);
        const forecastLineIdx = lines.findIndex(l => /^תוצאה צפויה:/i.test(l));
        
        const analysisLines = forecastLineIdx > -1 ? lines.slice(0, forecastLineIdx) : lines;
        const forecastLine = forecastLineIdx > -1 ? lines[forecastLineIdx] : '';
        ...
        return {
            ...
            forecast: forecast,
            predicted_score: predictedScore ? forecast : forecast,
