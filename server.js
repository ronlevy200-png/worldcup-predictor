...
1. סטטיסטיקות וביצועים עדכניים של כל נבחרת (ממוצע שערים, ספיגות, 5 משחקים אחרונים)
2. מצב שחקנים פצועים או מושעים מרכזיים
3. עד כמה המשחק משמעותי לכל קבוצה (השפעה על המשך הטורניר)
4. רמת עייפות / ימים מהמשחק הקודם
5. כוח ההתקפה והגנה של כל נבחרת

ענה בעברית בפורמט הבא בדיוק:
[עדכונים]
• שורה קצרה על מצב נבחרת ${home}
• שורה קצרה על מצב נבחרת ${away}
• עדכון פציעות/מצב שחקנים אם רלוונטי
• משמעות המשחק

[תחזית]
תוצאה צפויה: X-Y ל[שם הנבחרת] | [משפט הסבר קצר]`;
    ...
    try {
        ...
        // Parse the response
        const updatesMatch = text.match(/\[עדכונים\]([\s\S]*?)\[תחזית\]/);
        const forecastMatch = text.match(/\[תחזית\]([\s\S]*?)$/);

        const updates = updatesMatch ? updatesMatch[1].trim() : '';
        const forecast = forecastMatch ? forecastMatch[1].trim() : text;

        // Extract score prediction
        const scoreMatch = forecast.match(/(\d+)-(\d+)/);
        const predictedScore = scoreMatch ? `${scoreMatch[1]}-${scoreMatch[2]}` : '';
        const winnerMatch = forecast.match(/ל([^\|]+)/);
        const winner = winnerMatch ? winnerMatch[1].trim() : '';

        return {
            updates: updates,
            forecast: forecast,
            predicted_score: predictedScore ? `${predictedScore} ל${winner}` : forecast,
            ai_text: updates
        };
    } catch (e) {
