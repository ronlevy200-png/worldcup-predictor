...
async function generateAIAnalysis(home, away, stage, date) {
    ...
    const prompt = `אתה אנליסט כדורגל. כתוב ניתוח קצר בעברית על המשחק:

${home} נגד ${away}
שלב: ${stageName}

חוקים מחייבים:
- פסקה אחת רציפה בלבד, ללא כותרות, ללא bullets, ללא מספרים
- כתוב רק מה שאתה יודע על שתי הנבחרות: סגנון משחק, חוזקות, חולשות, שחקנים בולטים
- אל תכתוב שום הערה על מגבלות המידע שלך, על נתונים שחסרים לך, על פציעות שאינך יודע, על מה שיש לבדוק
- אל תכתוב ביטויים כמו: "להשגת ניתוח מדויק", "אין לי גישה", "יש לבדוק", "נכון לתאריך", "מצב שחקנים: אנא הערה"
- בסוף הפסקה, בשורה נפרדת, רשום: תוצאה צפויה: X-Y ל[שם הנבחרת]`;
...
// endpoint חד-פעמי לדריסת כל ניתוחי AI לפורמט החדש
app.post('/admin/reanalyze-all', async (req, res) => {
    try {
        const { data: matchList } = await sb.from('matches').select('id, home_team, away_team, match_date, stage').not('ai_analysis', 'is', null);
        if (!matchList || matchList.length === 0) return res.json({ ok: true, updated: 0 });
        
        let updated = 0;
        for (const m of matchList) {
            const analysis = await generateAIAnalysis(m.home_team, m.away_team, m.stage, m.match_date);
            if (analysis?.ai_text) {
                await sb.from('matches').update({
                    ai_analysis: analysis.ai_text,
                    owl_prediction: analysis.predicted_score
                }).eq('id', m.id);
                updated++;
                // המתן קצת בין בקשות כדי לא לעמוס על API
